  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-10 font-body">
      ...
      <div className="text-center mb-12">
        <span className="text-xs uppercase tracking-[0.3em] text-[#D4AF37] font-medium">Saved Treasures</span>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <span className="text-xs uppercase tracking-widest text-gray-500 font-medium">Accessing Jewellery Vault...</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {wishlistProducts.map((prod) => (
            <div
              key={prod.id}
              className="bg-white rounded-2xl border border-amber-100 overflow-hidden flex flex-col justify-between hover:shadow-lg transition-colors duration-200 relative group"
            >
              {/* Product Image Frame */}
              <div className="relative h-64 bg-gray-50 overflow-hidden">
                ...
                <button
                  ...
                  className="absolute top-3 right-3 p-2 bg-white/95 rounded-full border border-amber-100 text-red-500 hover:bg-red-50 active:scale-95 shadow-sm transition-transform duration-200"
                  title="Remove Saved"
                >
              ...
              <div className="p-5 flex flex-col gap-4">
                <div>
                  ...
                  <Link href={`/product/${normalizeSlug(prod.slug)}`} className="block">
                    <h3 className="text-sm font-medium text-[#1A1A1A] hover:text-[#D4AF37] transition-colors truncate mt-0.5">
                      {prod.name}
                    </h3>
                  </Link>
                ...
                <div className="flex items-center justify-between border-t border-amber-100/40 pt-4 mt-2">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-medium uppercase tracking-wider flex items-center gap-1.5 transition-transform duration-200 shadow-sm active:scale-95 border ${
                      prod.stock <= 0
                        ? 'bg-gray-100 border-amber-100 text-gray-400 cursor-not-allowed'
