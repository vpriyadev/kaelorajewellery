...
export default function CartPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-10 font-body">
      
      {/* Title */}
      <div className="text-center mb-12">
        <span className="text-xs uppercase tracking-[0.3em] text-[#D4AF37] font-medium">Shopping Bag</span>
      ...
      {cart.length > 0 ? (
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            ...
            <div className="bg-[#1A1A1A] border border-[#D4AF37]/50 rounded-2xl p-5 text-[#EDE6DA] shadow-lg flex items-start gap-4">
              ...
              <div className="flex-1">
                <h4 className="text-xs uppercase tracking-widest text-[#D4AF37] font-medium flex items-center gap-1">
                ...
                {isGiftUnlocked ? (
                  ...
                  <p className="text-xs mt-1.5 text-neutral-500 leading-relaxed font-body">
            ...
            <div className="flex flex-col gap-4 bg-white border border-amber-100 rounded-3xl p-4 sm:p-6 shadow-sm">
              {cart.map((item) => (
                <div
                  key={item.product.id}
                  className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 border-b border-amber-100/40 pb-4 mb-4 last:border-b-0 last:mb-0 last:pb-0"
                >
                  {/* Thumbnail frame */}
                  <div className="w-20 h-20 rounded-xl overflow-hidden border border-amber-100 bg-gray-50 flex-shrink-0">
                    <Image 
                  ...
                  <div className="flex-grow flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-4">
                    <div>
                      ...
                      <Link href={`/product/${normalizeSlug(item.product.slug)}`} className="block">
                        <h4 className="text-sm font-medium text-[#1A1A1A] hover:text-[#D4AF37] transition-colors">
                          {item.product.name}
                        </h4>
                      </Link>
                      <span className="inline-block bg-[#EDE6DA]/30 text-[#4B352A] text-xs uppercase tracking-wider px-2 py-0.5 rounded mt-1 font-medium">
                        {item.product.wearType} wear
                      </span>
                    </div>

                    {/* Quantity selectors */}
                    <div className="flex items-center border border-amber-100 rounded-xl overflow-hidden bg-gray-50/50">
                      <button
                        onClick={() => handleUpdateQty(item.product.id, item.quantity - 1, item.product.stock)}
                        className="px-2.5 py-1 text-sm font-medium border-r border-amber-100 hover:bg-gray-100 transition-colors"
                      >
                        -
                      </button>
                      <span className="px-3 text-xs font-normal w-10 text-center select-none">{item.quantity}</span>
                      <button
                        onClick={() => handleUpdateQty(item.product.id, item.quantity + 1, item.product.stock)}
                        className="px-2.5 py-1 text-sm font-medium border-l border-amber-100 hover:bg-gray-100 transition-colors"
                      >
                        +
                      </button>
                    </div>
          ...
          <div className="flex flex-col gap-6 w-full">
            <div className="bg-white border border-amber-100 rounded-3xl p-6 shadow-sm flex flex-col gap-5 sticky top-28">
              ...
              <div className="flex flex-col gap-3.5 text-xs text-gray-500">
                ...
                {shippingCharge > 0 && (
                  <div className="p-3 bg-[#F8F5F0] border border-amber-100 rounded-xl text-xs text-amber-700 leading-normal font-semibold">
              ...
              <Link
                href="/checkout"
                className="w-full py-3.5 bg-[#1A1A1A] hover:bg-[#2A2A2A] active:scale-[0.98] text-[#EDE6DA] text-xs font-medium uppercase tracking-[0.2em] rounded-xl text-center shadow-lg transition-transform duration-200 flex items-center justify-center gap-1.5 group border border-gray-800"
              >
              ...
              {/* Badges footer */}
              <p className="text-xs text-gray-400 text-center font-body mt-1">
        ...
        /* Empty Cart State */
        <div className="text-center py-20 bg-[#F8F5F0] border border-dashed border-amber-100 rounded-3xl p-8 max-w-lg mx-auto">
          <div className="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center mx-auto mb-4 border border-amber-100">
            <ShoppingBag className="w-7 h-7 text-gray-400 animate-bounce" />
          </div>
          <h2 className="text-xl font-display font-normal tracking-wide text-[#1A1A1A] uppercase tracking-wider">
            Your Cart is Empty
          </h2>
          ...
          <Link
            href="/shop"
            className="mt-8 px-8 py-3.5 bg-[#1A1A1A] text-white text-xs font-medium uppercase tracking-wider rounded-xl shadow-md inline-flex items-center gap-1.5 group active:scale-95"
          >
